...
SSDP.prototype._createSockets = function () {
  ...
  Object.keys(interfaces).forEach(function (iName) {
    self._logger.info('discovering all IPs from interface %s', iName)

    interfaces[iName].forEach(function (ipInfo) {
      if (ipInfo.internal == false && ipInfo.family == "IPv4") {
        self._logger.info('Will use interface %s', iName)
        var socket
  ...
  async.each(interfaces, function (iface, next) {
    ...
    socket.on('listening', function onSocketListening() {
      ...
      try {
        ...
        if (e.code === 'ENODEV') {
          self._logger.warn(e, 'Interface %s is not present to add multicast group membership. Scheduling a retry.', addr)
    ...
    socket.bind(self._ssdpPort, next) // socket binds on 0.0.0.0
  }, cb)
}
...
SSDP.prototype._notify = function (headers, _msg, _rinfo) {
  ...
  switch (headers.NTS.toLowerCase()) {
    ...
    case c.SSDP_ALIVE:
      this.emit(c.ADVERTISE_ALIVE, headers)
      break

    // Device shutting down.
    case c.SSDP_BYE:
      this.emit(c.ADVERTISE_BYE, headers)
      break
  ...
  Object.keys(self._usns).forEach(function (usn) {
    ...
    if (acceptor(usn, serviceType)) {
      ...
      self._logger.trace({
        'peer': peer_addr,
        'port': peer_port
      }, 'Sending a 200 OK for an M-SEARCH')
