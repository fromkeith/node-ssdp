...
SSDP.prototype._start = function (port, host, cb) {
  ...
  function startSocket() {
    chrome.sockets.udp.onReceive.addListener(function (info) {
      ...
      console.log('received:', String.fromCharCode.apply(null, new Uint8Array(info.data)));
  ...
  // first try close any existing sockets open on our desired port
  chrome.sockets.udp.getSockets(function (openSockets) {
    var i, waitForClose = 0, canStart = false;
    function socketClosed() {
      waitForClose--;
      if (canStart && waitForClose === 0) {
        startSocket();
      }
    }
    for (i = 0; i < openSockets.length; i++) {
      if (openSockets[i].localPort === port) {
        waitForClose++;
        chrome.sockets.udp.close(openSockets[i].socketId, socketClosed);
      }
    }
    if (waitForClose === 0) {
      startSocket();
